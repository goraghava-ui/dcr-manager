...
export function Sidebar({ active, onNav, role = "manager" }: {
  active: string;
  onNav?: (id: string) => void;
  role?: "manager" | "distributor";
}) {
  const navs = role === "manager" ? MANAGER_NAV : DIST_NAV;
  return (
    <aside style={{
      ...
      <nav style={{ display: "flex", flexDirection: "column", gap: 1 }}>
        <div style={{ fontSize: 10, color: "var(--ink-4)", textTransform: "uppercase" as const, letterSpacing: "0.08em", padding: "6px 10px 4px" }}>Workspace</div>
        {navs.map(n => (
          <div key={n.id} className={"nav-item " + (active === n.id ? "active" : "")}
            onClick={() => onNav?.(n.id)} style={{ cursor: "pointer" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "4px 6px" }}>
        ...
        <button className="btn btn-ghost btn-sm" style={{ width: 26, padding: 0, justifyContent: "center" }}>
...
export function PageHeader({ title, sub, actions, breadcrumb }: {
  title: string; sub?: string; actions?: ReactNode; breadcrumb?: string[];
}) {
  return (
        ...
        <div style={{ fontSize: 12, color: "var(--ink-3)", marginBottom: 6, display: "flex", alignItems: "center", gap: 6 }}>
          {breadcrumb.map((b, i) => (
            <span key={i} style={{ display: "flex", alignItems: "center", gap: 6 }}>
              {i > 0 && <Icon name="chevron" size={11} color="var(--ink-4)" />}
              <span>{b}</span>
            </span>
          ))}
        </div>
